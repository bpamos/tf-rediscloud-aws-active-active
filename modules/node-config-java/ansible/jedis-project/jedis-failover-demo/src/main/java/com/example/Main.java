...
public class Main 
{
    public static void main( String[] args )
    {
        ...
        for (int x = 1; x <= 100; x++){
            try {
                String key = connection + ":key:" + x;
                connection.incr("counter");
                String value = connection.get("counter");
                ...
                // Reconnect to Redis for the next iteration
                connection = JedisConnectionHelper.initRedisConnection(args);
            } catch(Throwable ste) {
                ste.printStackTrace();
            }
        }
        
        // Disconnect from Redis after completing all iterations
        connection.close();
    }
}
